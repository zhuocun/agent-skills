...
**In every brief.** Hold every delegate — worker, reviewer, verifier alike — to this same standard, and write that standard into its brief: it owns the quality of its own result rather than treating the next gate as the thing that catches its mistakes; it checks decisive claims at the source itself rather than reasoning from assumption or from what the brief handed it; and it re-reads its own output against the brief's scope and constraints before declaring done. Self-review is an addition to the independent reviewer hop, never a licence to skip it (see **Reviewer**) — a delegate's own sign-off is never acceptance.
