...
*A settled short form is not a clip.* Some short forms are the term rather than a compression of one — `压测`、`灰度`、`埋点`、`扩缩容`、`联调`、`打标`、`兜底`、`提测`、`自测`. Test one you have not met: can it stand on first mention as the head of its own sentence in a text where the long form never appears, and does it take compounds as a head (`全链路压测` / `压测平台` / `压测报告`)? Both yes, write the short form — 阿里云 PTS 的产品概述通篇用「压测」，全篇零次「压力测试」. Either no, write the full form. The trap is the clip that only trails its own long form in the same passage, or survives only as a modifier: `冒烟` and `回归` look settled but are this, so write `冒烟测试` and `回归测试` on first mention — and `重录`、`手维护` above are the same shape, which is why they are defects. `提测`、`自测`、`拉齐`、`放量` are internal-process register; use the full form facing outward. `回归` alone is ambiguous with 逻辑回归 wherever machine learning is in scope.
