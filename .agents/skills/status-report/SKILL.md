...
### 10. Render the HTML view — the markdown is the source, the page is a view

The markdown is the artifact people edit, diff, and quote; the page is generated from it and nothing on it is hand-maintained. That is the entire licence for drawing what the source may not — a redrawn value cannot drift from a value regenerated with it. Priority inside this pass: **the source's claims intact → legible at a glance → degrades gracefully → visual identity → polish.** Emit one self-contained file of plain HTML and CSS, portable to any destination.

- **Document order is slot order.** Print the seven slots top to bottom as the Form section fixes them, and render the Ledger's elements in the order pass 2 fixes — inside the Ledger section, never above its heading: the blocked-by line immediately after the heading, then the status-count line, then the graph, then the table.
- **Assign hierarchy one axis at a time, and spend each axis once.** Size: the masthead is the largest type, the verdict second. Chromatic weight: the blocked-by line alone carries it. Area: the Ledger table is the largest block. Every remaining slot takes body weight under a quiet uppercase label and competes on none of the three.
- **`Nothing blocked.` takes the quiet label form**, never the blocked-by line's alarm form. Rendering a clean bill of health as an alarm is a false alarm.
- **Give `Not checked.` a form `None.` does not have** — an outlined marker in the muted neutral, against `None.`'s plain body text. An unexamined slot that renders like an empty one destroys the distinction the Form section exists to hold.
- **The progress meter is discrete cells at a fixed cell width**: exactly denominator-many cells, numerator-many filled, so the track's *length* is the denominator and a five-unit workstream is visibly shorter than a twelve-unit one. This is pass 9's plain-text bar transferred to pixels, cell for cell. Never a continuous proportional fill: scaling a fill to its own denominator and normalising it to a percentage are the same arithmetic, so a fill is the shared cross-row scale pass 9 rejects.
- **The meter needs no script.** Denominator-many cells with numerator-many filled is static markup a reader verifies by counting — stronger than a promise that it was computed. Keep pass 9's ceiling of twelve on its own reason: past twelve, cells are too narrow to count.
- **A two-axis row stacks two lanes over one segmentation**, built and exposed, same denominator; give an all-empty lane a visible boundary so a finished-but-unexposed workstream reads as itself rather than as blank surface. A group row gets one segmented meter, segments in the attention order pass 2 fixes, set apart from a ratio meter in height or corner radius so the two are never read as one device.
- **Three type roles, no more: display, body, and data.** The data role carries every count, ratio, reference, and date, and uses tabular figures so columns of numbers align on their digits. A section label is a treatment of the display role, not a fourth face.
- **Colour carries pass 9's lifecycle ramp and the reserved blocked colour, and nothing else.** Make the ramp monotonic in lightness so it survives greyscale and colour-blind vision. Give the two off-ramp values a second channel besides hue — a texture or an outline — and never let colour be a state's sole carrier: the word in the Status cell is. Body text meets 4.5:1 against its background; large text and every non-text mark meet 3:1, computed rather than judged by eye.
- **Define the palette host-neutrally.** Put the complete set of tokens on bare `:root`; redefine only the changed tokens inside a `prefers-color-scheme: dark` query, guarded so an explicit light stamp on the root element overrides it; redefine them again under an explicit dark stamp so an explicit choice wins in both directions. A colour whose only definition sits inside a media or attribute block is a bug in the other theme, and `body` paints its own background token rather than inheriting the destination's. In dark, re-step the ramp rather than inverting it, so more progress still means more presence.
- **Match the destination's theme signal, or stand alone honestly.** Where the destination stamps a theme onto the root element, key the explicit-stamp selectors to that attribute. Where the page stands alone, either supply its own control or ship the media query alone — never selectors nothing will ever match.
- **Take one visual identity from the subject's own vernacular** — an accounts ledger, a lab notebook, an operations board — and carry it through every element. A status page that could be restyled into an unrelated product without changing anything has no identity. Restraint is the discipline applied to that identity, not a substitute for having one: one ground, one accent, one reserved alarm colour, and no ornament the source does not carry.
- **Avoid the looks that read as machine-generated**: warm cream with a serif display and a terracotta accent; near-black with one acid-green or vermilion pop; a purple-to-blue gradient hero; the safe geometric sans everyone reaches for; emoji as section markers; everything centred; an accent rail on a rounded card; a shadow on every surface; a grid of tiles with oversized coloured numbers.
- **Keep the semantics real.** The Ledger is a real table with a caption, column-scoped headers, and a row header per row — not a grid of boxes. Exactly one `<h1>`, the masthead. Every focusable control shows a visible focus state and no rule anywhere sets a bare `outline: none`. Honour a reduced-motion preference.
- **Degrade at the edges, not the centre.** The table sits in its own horizontally scrolling, keyboard-reachable container with a minimum width, so a narrow viewport scrolls the table instead of the page. Pair two slots into a two-column grid only where both rendered bodies are short, and never where either reads `Not checked.`; below the grid's breakpoint everything returns to one column in slot order.
- **A worked reference sits beside this file at `references/example.html`.** Copy its shape — the cell grammar, the empty-track boundary, how a meterless row reads, the token structure across the three theme states — and not its palette or its copy. It renders the example report in one direction; the rules above admit many.
- **Set no page-height budget.** The 40–50 line ceiling is a property of the markdown source; a rule measured in screens would reintroduce the screen dependency the Form section rejects.
...
- [ ] If a rendered view was produced: the seven slots print in Form order with nothing outside its own heading, and the blocked-by line, status-count line, and graph all sit inside the Ledger section in pass 2's order.
- [ ] Every meter is denominator-many cells at a fixed cell width with numerator-many filled and needs no script, absent where pass 9 omits the bar; two-axis rows stack two lanes with an empty lane still visible, and a group row's segmented meter is distinct from a ratio meter.
- [ ] Size, chromatic weight, and area are each spent once — masthead, blocked-by line, Ledger table; `Nothing blocked.` renders quiet, `Not checked.` renders distinctly from `None.`, and the page carries one identity from the subject's vernacular rather than a generic look.
- [ ] The complete palette sits on bare `:root` and is paired under both the guarded dark media query and the explicit dark stamp, `body` paints its own background, the ramp is monotonic in lightness with a second channel on the off-ramp values, contrast is computed at 4.5:1 body and 3:1 large and non-text, focus is visible, and the table scrolls inside its own keyboard-reachable container.
